...
abstract type A_Gaugefields{NC,Dim} <: AbstractGaugefields{NC,Dim} #Antihermitian matrix
end
...
function Base.:*(
    x::Array{<:A_Gaugefields{NC,Dim},1},
    y::Array{<:A_Gaugefields{NC,Dim},1},
) where {NC,Dim}
    s = 0.0
    for μ = 1:Dim
        s += x[μ] * y[μ]
...
function initialize_A_Gaugefields(U::Array{<:AbstractGaugefields{NC,Dim},1}) where {NC,Dim}
    F = Array{A_Gaugefields{NC,Dim},1}(undef, Dim)
    for μ = 1:Dim
        F[μ] = initialize_A_Gaugefields(U[μ])
    end
    return F
end
...
function initialize_A_Gaugefields(u::AbstractGaugefields{NC,Dim}) where {NC,Dim}
    return A_Gaugefields(NC, u.NX, u.NY, u.NT)
end
...
function gauss_distribution!(p::T; σ = 1.0) where {T<:A_Gaugefields}
    error("gauss_distribution!(p) is not implemented in type $(typeof(p)) ")
end
...
function gauss_distribution!(p::Array{<:A_Gaugefields{NC,Dim},1}; σ = 1.0) where {NC,Dim}
    for μ = 1:Dim
        gauss_distribution!(p[μ], σ = σ)
    end
end
...
function Base.setindex!(x::T, v, i...) where {T<:A_Gaugefields}
    error("setindex! is not implemented in type $(typeof(x)) ")
    x.a[i...] = v
end
...
function Base.getindex(x::T, i...) where {T<:A_Gaugefields}
    error("setindex! is not implemented in type $(typeof(x)) ")
    return x.a[i...]
end
...
function Base.similar(U::T) where {T<:A_Gaugefields}
    error("similar! is not implemented in type $(typeof(U)) ")
end
