...
function randomGaugefields_4D_wing(
    NC,
    NX,
    NY,
    NZ,
...
function Base.getindex(
    ...
    i5,
    i6,
) where {T<:Shifted_Gaugefields_4D,μ}
    NT = u.parent.NT
    NZ = u.parent.NZ
    NY = u.parent.NY
    NX = u.parent.NX
...
function construct_Λmatrix_forSTOUT!(
    Λ,
    δ_current::Gaugefields_4D_wing{NC},
    Q,
    u::Gaugefields_4D_wing{NC},
) where {NC}
    NT = u.NT
...
function exptU!(
    ...
    u::Gaugefields_4D_wing{NC},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_4D_wing,NC} #uout = exp(t*u)
...
function exptU!(
    uout::T,
    t::N,
    v::Gaugefields_4D_wing{2},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_4D_wing} #uout = exp(t*u)
    NT = v.NT
    NZ = v.NZ
    NY = v.NY
    NX = v.NX
