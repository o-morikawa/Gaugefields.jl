...
mutable struct Storedshiftfields{TG}
    _data::Vector{TG}
    _disp::Vector{Tuple{NTuple{4,Int}, Bool}}
    ...
    function Storedshiftfields(_data::Vector{TG}, _disp::Vector{Tuple{NTuple{4,Int}, Bool}}, _flagusing, _indices, _numused, Nmax) where {TG}
        return new{TG}(_data, _disp, _flagusing, _indices, _numused, Nmax)
    end
...
function is_stored_shiftfield(t::Storedshiftfields{TG}, l::Tuple{NTuple{4, Int}, Bool}) where {TG}
    if l in t._disp
        return true
    else
        return false
