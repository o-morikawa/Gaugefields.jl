...
function test_map_U2nhs_theta(x,y,t,m,n,rng,eps)
    ...
    if n==1
        r = [0.876608492574193]
    elseif n==2
        r = [0.876608492574193 0.5219642502018771; 0.08622342695413243 0.3779129543048858]
    elseif n==3
        r = [0.876608492574193 0.5219642502018771 0.08622342695413243; 0.3779129543048858 0.011644572969246703 0.9272661633421619; 0.5437567669510681 0.47933167047242464 0.24534922778070478]
    elseif n==4
        r = [0.876608492574193 0.5219642502018771 0.08622342695413243 0.3779129543048858; 0.011644572969246703 0.9272661633421619 0.5437567669510681 0.47933167047242464; 0.24534922778070478 0.7598960015615879 0.9849929975443044 0.21704512156333866; 0.4590171869250881 0.884729168704953 0.5838542859901488 0.2639731675000303]
    end
    theta[1,:,:] = ( m - 3 + cos(x) + cos(y) + cos(t) ) * r + eps*zeta[1,:,:]
    #+ ( m - 3 + cos(2x) + cos(2y) + cos(2t) ) * s

    if n==1
        r = [0.8400934725278035]
    elseif n==2
        r = [0.8400934725278035 0.005292539995173762; 0.20381352056978863 0.9537947095189847]
    elseif n==3
        r = [0.8400934725278035 0.005292539995173762 0.20381352056978863; 0.9537947095189847 0.41508206440865214 0.06390418562058486; 0.22123954438621207 0.29232260658369924 0.7070486600861496]
    elseif n==4
        r = [0.8400934725278035 0.005292539995173762 0.20381352056978863 0.9537947095189847; 0.41508206440865214 0.06390418562058486 0.22123954438621207 0.29232260658369924; 0.7070486600861496 0.7446005126738047 0.7520709207435481 0.9650767773527269; 0.5955762345784386 0.357155999761672 0.09745849794680561 0.5096183728199861]
    end
    theta[2,:,:] = sin(x) * r + eps*zeta[2,:,:]
    #+ sin(2x) * s
    if n==1
        r = [0.003912847405306286]
    elseif n==2
        r = [0.003912847405306286 0.6481849103764872; 0.32747967430726743 0.42958562996150196]
    elseif n==3
        r = [0.003912847405306286 0.6481849103764872 0.32747967430726743; 0.42958562996150196 0.44356528910750637 0.1523878038992892; 0.6707317883583468 0.6664470284756423 0.9739672993869939]
    elseif n==4
    ...
    theta[3,:,:] = sin(y) * r + eps*zeta[3,:,:]
    #+ sin(2y) * s
    if n==1
        r = [0.5219920025343154]
    elseif n==2
        r = [0.5219920025343154 0.09973719932020075; 0.9800628057498066 0.4856043179437335]
    elseif n==3
        r = [0.5219920025343154 0.09973719932020075 0.9800628057498066; 0.4856043179437335 0.2395294527440972 0.8179046610218443; 0.7923016581440165 0.27830885490902335 0.6121518504536243]
    elseif n==4
    ...
    theta[4,:,:] = sin(t) * r + eps*zeta[4,:,:]
    #+ sin(2t) * s
    
    return theta
end

function test_map_U2nhs_g(x,y,t,m,n,rng,eps)
    eye = [1.0+0.0im 0.0+0.0im; 0.0+0.0im 1.0+0.0im]
    sigma1 = [0.0+0.0im 1.0+0.0im; 1.0+0.0im 0.0+0.0im]
    sigma2 = [0.0+0.0im 0.0-1.0im; 0.0+1.0im 0.0+0.0im]
    sigma3 = [1.0+0.0im 0.0+0.0im; 0.0+0.0im (-1.0)+0.0im]

    theta = test_map_U2nhs_theta(x,y,t,m,n,rng,eps)
...
function test_map_U2nhsGaugefields_3D_nowing(NC, NX, NY, NT, m, n; verbose_level = 2, randomnumber = "Random", reps=0.1)
    @assert NC==2 "NC should be 2."
    U = Gaugefields_3D_nowing(NC*n, NX, NY, NT, verbose_level = verbose_level)
    if randomnumber == "Random"
        rng = MersenneTwister()
    ...
    for it = 1:NT
        for iy = 1:NY
            @inbounds @simd for ix = 1:NX
                ...
                y = - pi + (iy-1) * (2pi) / NY
                t = - pi + (it-1) * (2pi) / NT
                U[:,:, ix, iy, it] = test_map_U2nhs_g(x,y,t,m,n,rng,reps)
            end
        end
    end
    set_wing_U!
